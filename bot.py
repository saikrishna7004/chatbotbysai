...
def reply(message):
		...
		#print(message)
		#bot.send_message(chat, "Choose one letter:", reply_markup=markup)
		...
		if "hi"==text.lower() or "hello"==text.lower() or "namaste"==text.lower() or "hola"==text.lower():
			...
			if str(chat)=="903774501":
				...
				bot.send_message(chat, "Trying to `Send Post`...", parse_mode="Markdown")
			...
			if reply_type=="text":
				...
				bot.send_message(chat, "There is  `No Post`  to send.\n\nTo  `Send Post` , Click on  `Send Post to Subscribers`  for more details.", parse_mode="Markdown")
			else:
				bot.send_message(chat, "This type of  `File`  is not supported yet!", parse_mode="Markdown")
...
def handle_start(message):
	...
	text2="CAACAgIAAxkBAAIxRGBfVhR51koD9GWWdR0Pu1J_m0PCAAI4BQACP5XMCivNw0w_QtinHgQ"
	keyboard=keyfunc(message)
	bot.send_message(message.from_user.id, text1, reply_markup=keyboard)
...
@bot.message_handler(content_types=['document', 'photo', 'audio', 'voice', 'animation', 'gif', 'sticker'])
def handle_docs_audio(message):
	print(message.content_type)
	keyboard=keyfunc(message)
	bot.send_message(message.from_user.id, "You have sent a  `"+message.content_type.capitalize()+"` .", reply_markup=keyboard, parse_mode="Markdown")

# Handles all text messages that match the regular expression
@bot.message_handler(regexp="Saipythonbot")
def handle_message(message):
	bot.reply_to(message, "Did you call me?")
...
def webhook():
    bot.remove_webhook()
    bot.set_webhook(url='https://chatbotbysai.herokuapp.com/' + TOKEN)
    return "!", 200
